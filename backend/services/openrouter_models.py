    ...
    async def get_available_models(self, use_webpage: bool = False) -> List[Dict[str, Any]]:
        ...
        if api_models:
            ...
            for model in api_models:
                # Determinar preço
                pricing_obj = model.get("pricing", {})
                prompt_price = float(pricing_obj.get("prompt", 0))
                completion_price = float(pricing_obj.get("completion", 0))
                
                is_free = prompt_price == 0 and completion_price == 0
                
                # Fallback: verificar nome/id se não for zero (alguns free têm custo irrisório ou erro na API)
                name_lower = model.get("name", "").lower()
                id_lower = model.get("id", "").lower()
                
                if not is_free:
                    if "(free)" in name_lower or ":free" in id_lower or "free" in id_lower.split("/")[1:]:
                        is_free = True
