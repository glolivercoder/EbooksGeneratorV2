...
class LLMClient:
    ...
    async def generate(
        ...
        """
        print(f"Iniciando geração - Task: {task_type}, Model: {model}")
        print(f"OpenRouter Key: {'✓' if self.openrouter_key else '✗'}")
        print(f"OpenAI Key: {'✓' if self.openai_key else '✗'}")
        print(f"Gemini Key: {'✓' if self.gemini_key else '✗'}")
        ...
        if "gemini" in model.lower() and self.gemini_key:
            try:
                ...
                print(f"Gemini failed: {e}, falling back to other providers")
        ...
        if "gpt" in model.lower() or "openai" in model.lower() and self.openai_key:
            try:
                ...
                print(f"OpenAI failed: {e}, falling back to OpenRouter")
        ...
        for attempt_model in models_to_try:
            try:
                ...
                print(f"Timeout com {attempt_model}, tentando próximo modelo...")
                self._update_performance_cache(attempt_model, success=False)
                last_error = "Timeout"
                continue
                
            except Exception as e:
                print(f"Erro com {attempt_model}: {e}")
                last_error = str(e)
                continue
